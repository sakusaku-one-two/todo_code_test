...
4.  **自動マイグレーション**
    `go-migrate`の公式ドキュメントを参考に、`cmd/migrate/up/main.go`にマイグレーション処理を組み込みました。これにより、`migrate`のコマンドを手動で実行する必要がなくなりました。
