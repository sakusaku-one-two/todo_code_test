...
func testTodosInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(todoPrimaryKeyColumns, todoColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
var (
	todoDBTypes = map[string]string{`ID`: `int`, `Title`: `varchar`, `Description`: `text`, `LimitTime`: `datetime`, `CreatedAt`: `timestamp`, `UpdateAt`: `timestamp`, `StatusNo`: `int`, `IsActivate`: `tinyint`}
	_           = bytes.MinRead
)
